...
url_carregar_lotes_ano = "<?php echo base_url('lote/carregar_lotes_ano');?>";
	  	 ...
	  	 <div class="panel panel-default">
			...
			<div class="panel-body">
					...
					<table>
							<thead>
								<tr>
									<form class="form-inline form_lote" id="form_filtragem" method=post action="<?php base_url('lote/consultar_lote')?>">
										<th> 
											<label>Ano:</label>					
												<select class="form-control" id="filt_ano" name="filt_ano">
													<option value='0'> todos </option> 
													<? //$cont = $num_semana;
														foreach ($lista_anos as $i => $ano) { 
														$selected = ($ano_selec == $ano) ? 'selected' : '';	
										...
										<th> 	
											<label>Lote:</label> 										
												<select class="form-control" id="filt_lote" name="filt_lote">
													<option value='0'> todos </option> 
													<? //$cont = $num_semana;
														//foreach ($lista_lote as $cod_lote => $lote) { 
														//$selected = ($lote_selec == $lote) ? 'selected' : '';	
