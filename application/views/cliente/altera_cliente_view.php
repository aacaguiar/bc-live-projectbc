...
function carregar_cidade()
{
	...
	if(estado !='0' && estado !=''){
		...
		xmlhttp.open("POST","<?php echo base_url('cliente/carregar_cidades')?>",true);
...
<h4 class="text-center">Alterar Cliente <small>(pessoa física)</small> </h4>
 
<div class="box-cadastro-cli">
		<div class="panel panel-default">
			<div class="panel-heading">
				<span class="panel-title"><strong>Informações do cliente:</strong></span>
			</div>
			<div class="panel-body">
				<form name="cadastro_cliente" method="post" action="<?php echo base_url('cliente/alterar_dados_cliente');?>">
					<input type="hidden" name="cod_endereco" value="<? echo $endereco['cod_endereco'];?>">
					<input type="hidden" name="cod_fornecedor" value="<?php echo $dados['cod_fornecedor'];?>">
					...
					<div class="checkbox">
	    				...
	    				<?} else { ?>
	    						<input type="checkbox" id="check_fornecedor"  name="check_fornecedor" onClick="marcar_fornecedor()"> 
	    				<?} ?>
	    				
    					<label>	
    						Este cliente também é fornecedor.<?php echo ($dados['check_fornecedor']!='f') ? '(cód: '.$dados['cod_fornecedor'].' )' : ''; ?>
    					</label>
 					 </div>
  					...
  					<div class="form-group">
						...
						<input type="text" class="form-control" id="cod_fornecedor" name="cod_fornecedor"  size="15" value="<?php //echo $dados['cod_fornecedor'];?>">
					</div>
					-->
					
					<?php if($tipo_cliente == 'fisica') { ?>
						<div class="form-group">
