...
*/	
	...
	function consultar_lote($ano=0, $lote=0)
	{
		$retorno = $this->check_sessao();
		if ($retorno)
		{		
			$ano_selec = ($ano == 0) ? date("Y") : $ano;
			
			if( $this->input->post('filt_ano') != '' ) {
				$ano_selec = $this->input->post('filt_ano');
			}
			
			//$filt_lote = $this->input->post('filt_lote');
				
			//paginação
			$config['uri_segment'] = 5;
			$config['base_url'] = base_url('lote/consultar_lote/'.$ano_selec.'/'.$lote);
			$config['total_rows'] = $this->lote_model->total_linhas_lote($ano_selec, $lote);
			...
			$data = array(
					...
					'arquivo_css'	=> $arquivo_css,
					'lista_menu'	=> $lista_menu,
					'lista_anos'	=> $this->lote_model->lista_anos_cadastrados_lote(),
					'lista_lote' 	=> $this->lote_model->listar_lote($qtde, $inicio, $ano_selec, $lote ),           
	...
	*/
	...
	// carregar os lotes na view consultar lote
	function carregar_lotes_ano()
	{
		$filt_ano = $this->input->post('filt_ano');
		echo $filt_ano;
// 		if($filt_ano != 0)
// 		{
// 			$lista_lote_ano = $this->lote_model->carregar_lotes_ano($filt_ano);
			
// 			echo "<option value='0'>Todos</option>";
// 			foreach ($lista_lote_ano as $cod_lote => $num_lote)
// 			{
// 				echo "<option value='$num_lote' selected>".$num_lote."</option>";	
// 			}
// 		}
	}
	...
	/*
	 * Carrega lotes da view Controle do Lote
	*/
